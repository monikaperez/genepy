...
#' @param exclude_PRISM: Do you want to exclude PRISM data from the report
...
#' @param use_dep_unc Should dependency uncertainty estimates be used where available (D2)? (Default FALSE)
...
make_dep_report <- function(
  ...
  out_dir = NULL, save_pref_deps = FALSE,
  use_dep_unc = FALSE,
  nproc = 0) {
    library(rmarkdown)
    if (is.null(out_dir)) {
  ...
  if (exclude_PRISM) {
    target_dsets <- c('Achilles RNAi', 'DRIVE RNAi', 'Comb RNAi', 'CRISPR','GDSC', 'CTD2')
  } else {
    ...
    rmarkdown::render(input = system.file("rmd/depCon_template.Rmd", package="cdsr"),
                      ...
                      params = list(CL_var = CL_var,
                                    ...
                                    target_dsets = target_dsets,
                                    used_gscs = used_gscs,
                                    test_dir = test_dir,
                                    pref_deps_file = pref_deps_file,
                                    use_dep_unc = use_dep_unc,
                                    nproc = nproc))
}
