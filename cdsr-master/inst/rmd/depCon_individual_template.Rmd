...
### Volcano plot
Showing strength and significance of two-group comparison for each gene. 
...
### Distribution of cell line features by disease type
...
lin_ord <- df %>% filter(!is.na(Disease)) %>% dplyr::group_by(Disease) %>% dplyr::summarise(n = n()) %>% dplyr::arrange(dplyr::desc(n)) %>% .[['Disease']]
df %<>% mutate(Disease = factor(Disease, levels = lin_ord))

if (length(unique(CL_var[!is.na(CL_var)])) == 2) {
  ggplot(df %>% filter(!is.na(Disease)), 
         aes(x = Disease, fill = CL_var)) + 
...
with(envir, {
  ...
  for (gs_name in target_gs) {
   ...
   all_genes <- pref_dep_stats[[dset_name]][[target_type]]
    if (target_type == 'broad_id') {
      all_genes %<>% str_sub(1,13) %>% make.names()
    } else if (target_type == 'DRUG ID') {
      all_genes <-  paste0("ID", all_genes)
    }
})
```
