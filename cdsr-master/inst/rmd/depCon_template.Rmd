...
use_dep_unc <- params$use_dep_unc
...
data_info <- list(
  ...
  PRISM = list(
    ...
    transpose = F),
  GDSC_AUC = list(
    data.name = 'sanger-drug-sensitivities-d265',
    data.version = 2,
    data.file = 'GDSC_dr_v17_3',
    transpose = F),
  CTD2_AUC = list(
...
if (!('PRISM' %in% target_dsets)) {
  data_info <- data_info[setdiff(names(data_info), c('PRISM_annot', 'PRISM'))]
}
```
...
dat$sample_info %<>% dplyr::select(CCLE_ID = CCLE_name, Disease, Disease_subtype = `Disease Subtype`, Disease_subsubtype = `Disease Sub-subtype`) %>% 
  filter(!is.na(CCLE_ID)) %>% 
  dplyr::distinct(CCLE_ID, .keep_all=T)

#convert column names from Symbol (ID) to  Symbol
# to_gene_names <- c('RNAi_Comb_GS', 'RNAi_Comb_SE', 'RNAi_Ach_GS', 'RNAi_Ach_SE', 'RNAi_DRIVE_GS', 'RNAi_DRIVE_SE', 'CRISPR', 'CRISPR_FDR', 'RNAi_Ach_FDR')
...
#build AUC mat
GDSC_AUC_mat <- dat$GDSC_AUC %>%
      dplyr::select(full_drug_name, CCLE_ID, AUC) %>%
      acast(CCLE_ID ~ full_drug_name, value.var = 'AUC')
colnames(GDSC_AUC_mat) <- str_match(colnames(GDSC_AUC_mat), '\\(([0-9]+)\\)$')[,2]
dat$GDSC_AUC <- GDSC_AUC_mat
...
* **Distribution of cell line features by disease type**: Shows representation of cell lines in each dataset broken down by disease type and the specified cell line feature
...
if (debug_mode) {
  df_90_rank <- list(
    CRISPR = data.frame(Gene = colnames(dat$CRISPR)),
    `Achilles RNAi` = data.frame(Gene = colnames(dat$RNAi_Ach_GS)),
    `DRIVE RNAi` = data.frame(Gene = colnames(dat$RNAi_DRIVE_GS)),
    `Comb RNAi` = data.frame(Gene = colnames(dat$RNAi_Comb_GS)),
    `GDSC` = data.frame(`DRUG ID` = colnames(dat$GDSC_AUC), check.names = F),
    `CTD2` = data.frame(`master_cpd_id` = colnames(dat$CTD2_AUC))
  )
} else {
    df_90_rank <- list(
...
if ('PRISM' %in% target_dsets) {
  df_90_rank$PRISM <- cdsr:::get_df_90_rank(dat$PRISM, 'broad_id')
...
if (!use_dep_unc) {
  dset_tables %<>% lapply(function(xx) {
    xx$dep_SE <- NULL
    return(xx)
  })
}
if ('PRISM' %in% target_dsets) {
    dset_tables$PRISM <-list(dep_mat = dat$PRISM,
                dep_SE = NULL,
...
for (dset_name in target_dsets) {
   ...
   cur_dep_FDR <- dset_tables[[dset_name]]$dep_fdr
     pref_dep_stats[[dset_name]] <- tryCatch({cdsr:::get_limma_stats(cur_dep_mat, CL_var, covars, cur_dep_SE, dset_tables[[dset_name]]$target_type, test_dir = test_dir)},
                                             error = function(e) { #else return empty dataframe with same structure
                                               df <- data.frame(
                                                 p_val = numeric(),
                                                 q_val = numeric(),
                                                 EffectSize = numeric(),
                                               ...
                                               df[[dset_tables[[dset_name]]$target_type]] <- 
                                                 character()
                                               return(df)
                                             })
   ...
   if (is_two_group) {
     pref_dep_stats[[dset_name]] %<>% filter(min_samples >= min_CLs_per_group)
    ...
    pref_dep_stats[[dset_name]] %<>% left_join(
      ...
      pref_dep_stats[[dset_name]] %<>% dplyr::select(-min_samples)
     if (!is.null(FDR_deps)) {
       pref_dep_stats[[dset_name]] %<>% left_join(
...
Achilles_hits <- pref_dep_stats$`Achilles RNAi` %>% 
                filter(q_val < q_thresh) %>% 
                .[['Gene']]
 DRIVE_hits <- pref_dep_stats$`DRIVE RNAi` %>% 
                filter(q_val < q_thresh) %>% 
                .[['Gene']]
 CRISPR_hits <- pref_dep_stats$CRISPR %>% 
                filter(q_val < q_thresh) %>% 
                .[['Gene']]
gene_pref_deps <- list(
  Achilles = Achilles_hits,
  DRIVE = DRIVE_hits,
  CRISPR = CRISPR_hits
)
...
DT::datatable(df_tb,
      options = list(lengthMenu = c(5, 10, 25, 50, 100), 
                     pageLength = 25,
                     columnDefs = list(list(targets = c(10, 11, 12, 13), visible = FALSE))),
...
### GSA, common hits {.tabset .tabset-fade}
...
for (gs_name in target_gs) {
  print(gs_name)
    gsea_res[[gs_name]] <- tryCatch({gsea_res[[gs_name]] <- cdsr::run_GSAhyper(common_hits, all_genes,gsc = gsc_data[[gs_name]])
    gsea_res[[gs_name]] %<>% arrange(`p-value`)
    return(gsea_res[[gs_name]])
...
cur_out <- NULL
for (gs_name in target_gs) {
  cur_out <- c(cur_out, knit_child(system.file("rmd/depCon_gs_common_template.Rmd", package="cdsr")))
}
```
...
for (dset_name in target_dsets) {
  ...
  if (nrow(pref_dep_stats[[dset_name]]) > 0) {
    if (debug_mode) {
   out <- c(out, knit_child('~/CPDS/packages/cdsr/inst/rmd/depCon_individual_template.Rmd', envir = parent.frame()))
    } else {
