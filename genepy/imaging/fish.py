...
def drawDots(dists, scenter=False, size=1000, zsize=1000, 
						 ...
						 area="area", norm=None, second=None,
	...
	for i, (k,a) in enumerate(dists.items()):
		...
		ax=sns.kdeplot(data=a[['x', 'y', signal]].astype(float),
								 x='x', fill=True, y='y', cbar=False, weights=signal, color="seagreen", 
								 shade_lowest=False, levels=levels, hue_norm=(None, 10))
		if second is not None:
			print('adding second color')
			sns.scatterplot(data=a[second(a)], x='x', y='y', hue_norm=(None, max(sm)/sca) if norm is None else norm,
